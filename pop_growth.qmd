...
## Part 1: Let's start with exponential population growth {background-color="#272822"}
...
::::
:::: {.column width="50%"}
Logarithm of exponential growth
::::
:::
...
ggplot(pop_dat1, aes(x = time, y = spp))+
  ...
  annotate("text", x = 60, y = c(400), label = c("r = 0.05"))+
  ylab("population size (N)")+
  theme_minimal()

ggplot(exp_pop_dat, aes(x = exp_pop, y = growth_rate))+
  geom_line(show.legend = FALSE, colour = "red")+
  annotate("text", x = 500, y = c(40), label = c("r = 0.05"))+
...
## Take 5 minutes to discuss the assumptions of the exponential growth model {background-color="#272822"}

::: {style="text-align: center;"}
( ͡° ͜ʖ ͡°)
:::

## Part 2: Logistic population growth {background-color="#272822"}
...
__Unlike__ the exponential model, in the logistic model, growth rate is dependent on population size and reaches its peak at half of the carrying capacity
...
ggplot(logistic_pop_dat, aes(x = logistic_pop_dat, y = growth_rate))+
  geom_line(show.legend = FALSE, colour = "black")+
  annotate("text", x = 500, y = c(15), label = c("r = 0.05"))+
  xlab("Population size (N)")+
  ylab("rate of change (dN/dt)")+
  theme_minimal()
```
...
## Take 5 minutes to discuss the assumptions of the logistic growth model {background-color="#272822"}

::: {style="text-align: center;"}
(ᵔᴥᵔ)
:::

## Part 3: Introducing stochasticity {background-color="#272822"}
...
stochastic_pop <- bind_rows(stochastic_pop, .id = "run_id")


stochastic_pop_high <- replicate(10, r_stochastic.fun(r_mu = 0.02, r_sd = 0.3, K = 1000, N_pop = 300, t = 250), simplify = FALSE)
stochastic_pop_high <- bind_rows(stochastic_pop_high, .id = "run_id")
...
## Go forth and model!!! {background-color="#272822"}
